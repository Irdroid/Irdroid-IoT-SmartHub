...
private ImageAreasActivity im;
	...
	public boolean sendIrCommand(String remote, String command, int count,
		...
		try {
			return setTransmitters(transmitter)
					&& sendCommand("SIMULATE " + remote + " " + command + " "
							+ (count - 1), false) != null;
	...
	public boolean sendIr1Command(String remote, String command, int count)
		...
		try {
			...
			sendCommand("SIMULATE " + command + " " + remote
					, false);
		} catch (IOException ex) {
           
			Toast.makeText(im.getApplicationContext(),"Error connecting",Toast.LENGTH_SHORT).show();
	 ...
	 * Exception { return send_command("SIMULATE " + remote + " " + cmd + " " +
	 * (count - 1), false) != null; }
	 */
