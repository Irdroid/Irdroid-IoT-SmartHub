...
#endif
			{
				if (sock_count > 0)
				{
					...
					//	websocket_mask_data( sock_buf, out_buf, sock_count );
						...
						memset( sock_buf,0, sizeof( sock_buf ) );
					//	memcpy( sock_buf, &out_buf[21], sock_count-21 );
					//	DBG_MSG3("Server %d .Unmasked data - %s",port, sock_buf);
					...
					DBG_MSG3("server(%d) - wrote %s bytes to sio", port, sock_buf);
					sock_count -= res;
				}
			}
		// fprintf(stderr,sock_buf); Clear sockbuf
//		memset((void *)sock_buf, 0, sizeof(sock_buf) ); 
